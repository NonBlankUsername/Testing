...
void CEntities::Update() {
    ...
    for (int i = 0; i < 64; ++i) {
        ...
        // Loop through bone connections and store their positions
        for (int i = 0; i < 18; ++i) {
            Vector3 BonePos = *reinterpret_cast<Vector3*>(CurrentEntity.Base); 
            CurrentEntity.Bones[i] = BonePos; 
        }
...
bool CEntities::IsValidEntity(const EntityData& Data, int LocalTeam) const {
    ...
    const auto LocalPawn = *reinterpret_cast<uintptr_t*>(Client + Offsets::LocalPlayer);
    if (Data.Pawn == LocalPawn) return false;  // Skip the local player
    const auto EntityTeam = *reinterpret_cast<int*>(Data.Pawn + 0x3E3);
    const auto LocalPlayerTeam = *reinterpret_cast<int*>(LocalPawn + 0x3E3);

    if (Globals::Misc::TeamCheck && EntityTeam == LocalPlayerTeam) {
        return false;
    }
